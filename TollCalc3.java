...
Correct
timeHour is assigned with 5. 5 < 6 is true, so the first branch executes, and tollAmount is assigned with 1.55.
2)
9:45

Correct
timeHour is assigned with 9. 9 < 6 is false, so the first branch does not execute. Then, 9 < 10 is evaluated, which is true. The second branch executes, and tollAmount is assigned with 4.65.
3)
10:00

Correct
timeHour is assigned with 10. 10 < 6 is false, and 10 < 10 is false, so the first two branches do not execute. Then, 10 < 18 is evaluated, which is true. Thus, the third branch executes, and tollAmount is assigned with 2.35.
4)
22:15

Correct
timeHour is assigned with 22, which is not less than 6, 10, or 18. Thus, the else branch executes and assigns tollAmount with 1.55.
...
Correct
The outer if-else statements checks if typeOfDay is 0, in which case the nested if-else statements implement the weekday toll schedules. If typeDay is not 0, the else part's nested if-else statement implements the toll schedule for weekends and holidays.
2)
...
Correct
Because the two toll schedules are not the same, this approach would have 10 branches in the outer if-else statement, and 2 branches in each nested if-else statement, for a total of 30 branches. The current implementation has 2 branches in the outer if-else statement, 4 branches in one of the nested if-else statements, and 6 branches in the other nested if-else statement, for a total of only 12 branches.
3)
If timeHour is 0 and timeMinute is 30, the time will be output as: 0:30.

Correct
The if-else statements at the end of the program display the time using am/pm format. timeHours is 0, so the program prints 12 for the hour and 30 for the minutes, yielding 12:30.
